...
function printSummary({
  ...
  printWindowsAutologonNotice(mode);
}

/**
 * Требование автовхода на Windows.
 *
 * Кластер и media-service поднимаются задачами планировщика с триггером
 * «при входе пользователя»: `-AtStartup` может зарегистрировать только
 * администратор, а PostgreSQL на Windows вдобавок отказывается работать из
 * процесса с административными правами. Значит без автовхода перезагруженная
 * машина остаётся без базы и без эфира — и заметно это станет уже в линии,
 * поэтому мастер говорит об этом сам и последней строкой.
 */
function printWindowsAutologonNotice(mode) {
  if (process.platform !== "win32" || mode !== "production") return;
  console.log("\n  ВНИМАНИЕ: включите автовход в Windows.");
  console.log("  База и эфир поднимаются при входе пользователя. Без автовхода после");
  console.log("  перезагрузки машина останется без эфира до тех пор, пока в неё не войдут.");
  console.log("  Win+R → netplwiz → снять «Требовать ввод имени пользователя и пароля».");
}
