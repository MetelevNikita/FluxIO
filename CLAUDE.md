...
- **Опрос статуса каждые 750 мс перерисовывает всё дерево приложения.** Любой массив или
  функция, собранные прямо в JSX `App.tsx`, меняют идентичность на каждом опросе и тянут за
  собой перерисовку тяжёлых экранов. Новые пропсы к `EffectsScreen` заворачивай в `useMemo` /
  `useCallback`, а тяжёлые поддеревья — в `memo`.
