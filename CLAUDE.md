...
`apps/media-server/src/audio/tracks.ts`, таблица языков → ISO 639-2 — в `audio/languages.ts`.
...
- **`apps/web/package.json` перечисляет тестовые файлы вручную.** Новый `*.test.ts` в `apps/web/src`
  скомпилируется, но `npm test` его не запустит, пока не добавишь путь `dist-test/...` в скрипт
  `test`. Тихо зелёная сборка при неработающем тесте.
- **Сгенерированный Prisma-клиент одновременно в `.gitignore` и в индексе git** (15 файлов
  ...
  противоречиво.
- **Версия в дереве (6.0.22) отстаёт от последнего коммита** `3e8d7df 6.0.23 fix hot reloader`:
  строки версии не были подняты. Перед следующим релизом сверься.
