...
`Final transport preview started ... post-TSDuck MPEG-TS mirror` подтверждает
старт контрольной петли. Этот monitor не заменяет приёмный контроль ГС: потери
на NIC/switch/VLAN видны только на независимом приёмнике или return feed.
...
Версия текущего этапа — `v6.0.20`. Каждый следующий завершённый update увеличивает patch на единицу; подробности — в `docs/versioning.md`.
