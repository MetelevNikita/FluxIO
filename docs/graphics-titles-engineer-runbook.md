...
1. В After Effects соберите композицию и экспортируйте её панелью **FluxIO
   Title Studio**. Обычный экспорт Bodymovin с v8.0.0 не принимается: в нём нет
   метки `meta.flux`, то есть неизвестно, какие слои объявлены редактируемыми и
   с какими полями JSON они связаны.
2. В `Effects` создайте эффект нужного вида и в блоке `Оформление` выберите
   `Шаблон`, затем загрузите JSON кнопкой рядом. Общего импорта графики больше
   нет — файл принадлежит эффекту.
...
6. `Add to entire project` добавляет новый экземпляр эффекта во все ролики
   Current и Future при каждом нажатии. Для одного материала выберите его в
   selector и нажмите `Add to clip`; повторное нажатие создаст ещё один
   независимый слой того же эффекта.
