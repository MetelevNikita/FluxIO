...
Статус: завершён 2026-08-18. Отчёт:
[`progress/04-24-multitrack-audio-length-preview-restore-v6.0.24.md`](progress/04-24-multitrack-audio-length-preview-restore-v6.0.24.md).

## Этап 5. Эфирные эффекты второго уровня

### 5.1. Animation in/out, Next program, Ticker crawl, Clock/countdown, Stinger v7.0.0

Вкладка `Effects` разделена на три уровня графики. Уровень 2 — пять
параметрических эфирных эффектов с собственным поведением; они разрешаются в
FX-слои, динамические надписи и звуковые вставки, поэтому эфирный контур про
уровни не знает. Stinger режется по `Cut point` через стык роликов, часы идут
по эфирному времени, звук перехода подмешивается до хвоста, фиксирующего длину
дорожки. Отчёт —
[`progress/05-01-broadcast-effects-v7.0.0.md`](progress/05-01-broadcast-effects-v7.0.0.md).
Инструкция —
...
Суточный журнал на рабочем столе машины: работа эфира, смена роликов, транспорт
и ошибки с датой и временем; в конце дня — сводный отчёт. Потерянная графика
расписания разбирается диалогом выбора замены вместо отказа на Start. Часы идут
по времени устройства. Отчёт —
[`progress/05-02-logging-recovery-ui-fixes-v7.0.1.md`](progress/05-02-logging-recovery-ui-fixes-v7.0.1.md).

### 5.3. Бегущая строка, шрифты, предпросмотр и отзывчивость интерфейса v7.0.2

Вкладка Effects больше не перерисовывается на ежесекундном опросе статуса —
селекторы остаются рабочими во время эфира. Бегущая строка двигается и снимается
прямо из плейлиста, берёт новости из RSS и рисуется выбранным системным шрифтом
с проверкой кириллицы. У каждого эффекта появился предпросмотр. Отчёт —
[`progress/05-03-ticker-fonts-preview-v7.0.2.md`](progress/05-03-ticker-fonts-preview-v7.0.2.md).

### 5.4. Имена шрифтов, ключи пресета, отсчёт по ролику и отзывчивость v7.0.3

Имена шрифтов декодируются правильно (были иероглифы из-за перестановки символов
вместо байтов). Ключ текстового поля Next program выбирается из реального набора
пресета, а состав полей виден в инспекторе. Обратный отсчёт умеет считать до
конца ролика. Устранены две причины, по которым media-service переставал
отвечать; заминки пишутся в журнал. Отчёт —
[`progress/05-04-font-names-preset-keys-responsiveness-v7.0.3.md`](progress/05-04-font-names-preset-keys-responsiveness-v7.0.3.md).

### 5.5. Отсчёт от конца ролика на импортированном расписании v7.0.4

Нулевой `durationSeconds` неразобранного файла обнулял длительность ролика, и
всё, что отсчитывается от его конца, уезжало в начало. Отчёт —
[`progress/05-05-air-duration-offset-v7.0.4.md`](progress/05-05-air-duration-offset-v7.0.4.md).

### 5.6. Ввод, кнопка Save и пресеты во всех эффектах v7.0.5

Числовое поле больше не схлопывается в ноль на незавершённом вводе, набор текста
развязан от глобального состояния. Кнопка `Save` переносит правки в уже
назначенные ролики. Выбор и подгрузка Lottie-пресета есть у всех пяти эффектов,
включая стингер. Интерфейс предупреждает о несовпадении версии с media-service.
Отчёт —
[`progress/05-06-input-lag-save-presets-v7.0.5.md`](progress/05-06-input-lag-save-presets-v7.0.5.md).

### 5.7. Раскладка инспектора, подписи на пресете и плотность чипов v7.0.6

Панель настроек больше не уходит нижним краем за область прокрутки — элементы
в этой полосе не принимали клики. У Ticker и Clock появилась подпись в текстовом
поле пресета, у Save — подтверждение, у чипов графики — колонки без роста высоты
строки. Отчёт —
[`progress/05-07-inspector-layout-captions-chips-v7.0.6.md`](progress/05-07-inspector-layout-captions-chips-v7.0.6.md).

### 5.8. Живое значение внутри Lottie-плашки v7.0.7

С текстового слоя пресета снимается геометрия и оформление, поле шаблона
очищается, а на его место встаёт живое значение эффекта — часы, отсчёт, бегущая
строка. Отчёт —
[`progress/05-08-live-value-inside-lottie-v7.0.7.md`](progress/05-08-live-value-inside-lottie-v7.0.7.md).

### 5.9. Один эффект — один чип, порядок эффектов перетаскиванием v7.0.8

Плашка и живое значение показываются одним чипом с пометкой состава, снимаются
вместе. Карточки эффектов переставляются перетаскиванием. Отчёт —
[`progress/05-09-grouped-chips-reorder-v7.0.8.md`](progress/05-09-grouped-chips-reorder-v7.0.8.md).

### 5.10. Бегущая строка внутри плашки v7.0.9

У строки появилась полоса с настоящей обрезкой: она рисуется на отдельном
холсте и не выходит за пределы плашки. Отчёт —
[`progress/05-10-ticker-region-v7.0.9.md`](progress/05-10-ticker-region-v7.0.9.md).

### 5.11. Почему привязанная строка не видна v7.0.10

Привязанная к плашке строка наследует её цвет и без ограничения полосой уезжает
за плашку — план предупреждает об этом, а полоса показана в предпросмотре.
Отчёт —
[`progress/05-11-ticker-binding-diagnostics-v7.0.10.md`](progress/05-11-ticker-binding-diagnostics-v7.0.10.md).

- рендерер каждой дорожки отдаёт ровно длину ролика: `apad` + `atrim=end_sample`;
- короткий файл перевода больше не останавливает мультиплексор и весь эфир;
- supervisor дописывает недостачу тишиной и пишет об этом в лог эфира;
- `scanAudioTracks` измеряет длительность найденных файлов через ffprobe;
- полосы дорожек выровнены по левому краю кадров превью и заполняются вправо;
- media-service заново анализирует незнакомый файл, поэтому превью не слетают
  после перезапуска приложения.

### 5.12. Этапы v7.0.11 — v7.0.17

Динамическая плашка, форматы анимированного логотипа, отзывчивые подложки
`fit:`, JSON Parser, экспортёр титров для After Effects, пакетное применение
Animation In/Out из JSON и надёжность экрана Effects. Отчёты по каждому этапу —
в [`progress`](progress), файлы `05-12-…` — `05-18-…`.

### 6.1. Собственные титры вместо Lottie v8.0.0

Связка с After Effects и Lottie убрана целиком: титры рисует собственная сцена —
дерево узлов внутри эффекта. Причина не в удобстве, а в отказе на эфире: плашка
была отрендеренным файлом, текст поверх неё рисовал `drawtext`, и на стыке текст
успевал появиться раньше подложки. Теперь оба рисуются одним проходом, одной
функцией, которую вызывают и редактор, и эфир. Отчёт —
[`progress/06-01-effects-library-rework-v8.0.0.md`](progress/06-01-effects-library-rework-v8.0.0.md).

- один шаблон работает на четыре раскладки (4:3, 16:9, HD, UHD) и обе частоты
  (25 и 50 полей): пересчёт идёт от долей кадра;
- рисуется только область показа, а не весь кадр — полный кадр 2160 RGBA это
  829 МБ/с и в реальное время не укладывается;
- время кадра берётся из его номера, а не из системных часов: рендерер
  следующего ролика запускается заранее;
- оформление файлом осталось у Animation in/out и Stinger — им нужен целый
  видеоряд, а не плашка;
- общий импорт графики и выбор пресета из списка убраны;
- список эффектов фильтруется, но никогда не сортируется: порядок добавления
  задаёт порядок наложения слоёв;
- незавершённый эффект виден в списке, применение заблокировано;
- частота кадров последовательности `.png` задаётся оператором, пропуски в
  нумерации показываются до применения;
- пустое расписание поднимает эфир на цветных полосах;
- мастер установки первым вопросом спрашивает про офлайн-режим и хранит две
  последние копии `.env`.

### 6.2. Редактор титров

Собран. Раскладка как у эфирных титровальников — слои слева, холст в середине,
свойства справа, время внизу, — плюс переключатель раскладочных целей, которого
у них нет: один шаблон обязан выйти в 4:3, 16:9, HD и UHD.

- холст рисует той же функцией, что и эфир, поверх реального кадра из плейлиста;
- направляющие, прилипание и безопасные зоны вещания; порог прилипания задан
  в пикселях экрана, а не в долях кадра;
- перетаскивание анимированного узла двигает всю дорожку, а не базовое значение;
- дорожка ключей с тремя отрезками режиссёра; в удержании ключей не бывает;
- объявление полей: ключ выводит редактор из подписи, руками его не набирают;
- проверки шаблона до эфира — осиротевшая привязка, необъявленное поле,
  невыбранный шрифт, текст за зоной надписей.

Макет — файл Figma «FluxIO — Редактор титров». План фаз —
[`title-editor-plan.html`](title-editor-plan.html).

### 6.3. Аппаратное кодирование

Сделано. Эфир кодируется выбранным ускорителем: NVENC, Quick Sync, AMF, VAAPI
или VideoToolbox; «Авто» берёт первый доступный. Список в интерфейсе строится
по тому, что реально есть в сборке FFmpeg на машине.

- подмены на программный кодировщик не бывает: недоступный ускоритель — отказ
  preflight с именем кодировщика, потому что тихий откат вылез бы в эфире
  пропущенными кадрами;
- чересстрочность умеют не все — отдать прогрессив вместо 50i значит не совпасть
  с профилем, и это ловится до старта;
- предпросмотр всегда программный: ускоритель у машины один, и предпросмотр,
  взяв его, отбирал бы ресурс у эфира;
- VAAPI кодирует в памяти ускорителя, поэтому ему дописывается ступень
  `format=nv12,hwupload` в тот же граф фильтров.

Проверено на живом эфире через VideoToolbox: поток H.264 High, предпросмотр
остался на `libx264`. NVENC, Quick Sync, AMF и VAAPI собраны по документации
и проверены тестами на состав аргументов — железа для живого прогона нет.

### 6.4. Титры: кривые, подложки, раскладки

- **Кривые Безье** у ключей анимации — та же математика, что у `cubic-bezier()`
  в CSS и графика скорости в After Effects; кривая принадлежит ключу, к которому
  идёт.
- **Подложка из видео с альфой или `.png`-последовательности**: разрешается в
  обычный FX-слой и попадает в кадр через FFmpeg, а не через растеризатор сцены.
  Длина показа подгоняется под источник.
- **Разрешения шаблона** спрашиваются отдельным окном при создании: от набора
  целей зависит работа дизайнера, а незаявленная раскладка в эфир не пойдёт.
- **Поля шаблона** видны в инспекторе эффекта с образцами; файл задания
  принимает любой эффект, чья сцена объявила поля, — раньше список видов был
  зашит, и собранный титр с полями JSON не принимал.

### 6.5. Титры: слои, группы и появление текста v8.0.1

- **Таймлиния показывает все слои**, а раскрывает только анимированные свойства:
  свёрнутый слой отдаёт сводную полосу с положением ключей. Дорожек стало
  одиннадцать — добавились `scaleY`, `skewDegrees` и `blur`.
- **Группа-контейнер** берёт размер от ребёнка (обычно от подложки) и режет
  содержимое по своим границам: одно раскрытие группы прячет весь текст разом,
  вместо ключа на каждый слой.
- **Появление текста по частям** — буквы, слова или строки со сдвигом по
  времени; семь готовых наборов, первые из набора After Effects. Разнос задан
  долей, а не секундами: волна укладывается в длину входа при любой длине
  строки.
- **Отмена по шагам** с окном склейки 400 мс — иначе один Ctrl-Z откатывал бы
  каждое движение мыши по отдельности.
- **Точка привязки и точка среза маски** — сеткой 3×3, как в After Effects;
  перенос привязки не двигает узел и правит всю дорожку.
- **Файл `.fto`**, папка `FluxIO Titles` на рабочем столе и базовый набор из
  шести новостных титров, поставляемый с программой.
- **Автостарт после запуска** поднимает эфир с прерванного места — с видимым
  обратным отсчётом и отменой.
- **Одно превью эффекта** вместо двух: сцену рисует та же функция, что и эфир,
  с проигрыванием и перемоткой; макет положения остался только у оформления
  файлом.
- **Имя эффекта** правится полем в шапке и двойным щелчком в списке.
- **Градиентная заливка** узла — линейная и радиальная, с прозрачностью у каждой
  точки перехода; точки заданы долями коробки узла, а не кадра.
- **Три способа применения** в одном месте: на ролик, на весь проект и по файлу
  задания. Значения титра берутся по полям сцены, связка с JSON держится на
  совпадении имён, ключ сопоставления выбирается по значениям.

Отчёт — [`progress/06-02-title-editor-v8.0.1.md`](progress/06-02-title-editor-v8.0.1.md).
...
- black/slate fallback при ошибке файла (пустое расписание уже закрыто цветными полосами v8.0.0);
