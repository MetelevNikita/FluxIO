...
### 6.1. Перестройка библиотеки эффектов v8.0.0

Уровень 3 перестал быть самостоятельным элементом списка: графика стала
свойством конкретного эффекта. Оформление — явный выбор «шаблон или плашка».
Шаблоны титров принимаются только из FluxIO Title Studio. Стингер берёт `.mov`
с альфой или последовательность `.png`. Пустое расписание поднимает эфир на
цветных полосах. Отчёт —
