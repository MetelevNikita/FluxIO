...
Одна установка может поднять несколько таких каналов. Это не несколько
supervisor-ов внутри одного процесса, а несколько одинаковых media-service:
у каждого свой `PlayoutSupervisor`, API-порт, `.env` и база PostgreSQL. Electron
показывает общий launcher и открывает по операторскому окну на выбранный API.
Падение одной программы поэтому не уносит остальные.
