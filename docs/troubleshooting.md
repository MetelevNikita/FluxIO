...
### Локальный адрес (127.0.0.1, localhost)

Это отдельный случай: firewall и головная станция ни при чём — приёмника просто нет на
этой же машине. Два способа проверить выдачу локально:

```bash
ffplay "srt://127.0.0.1:9000?mode=listener"
```

Запустите приёмник **до** старта эфира: FluxIO в режиме caller звонит один раз и ждать
никого не будет. Либо наоборот — переключите выход в режим listener, тогда FluxIO ждёт
подключения, а плеер звонит ему:
...
## Один из нескольких выходов упал

Откройте Broadcast → **Потоки и ресурсы**. Ошибка и состояние относятся только
к этой ветке; программа и соседние выходы продолжают работу. Исправьте адрес,
приёмник или firewall, затем нажмите Start у ветки. Если нужно изменить саму
конфигурацию выхода, остановите общую сессию: зеркала и порты фиксируются при
её старте.

При высокой нагрузке сначала отличите `relay` от `transcode`. Relay почти не
кодирует, а каждый transcode — отдельный FFmpeg. Сравните CPU веток, общий CPU
эфирной цепочки и CPU системы в окне «Программы FluxIO».
