...
## До трёх выходов одной программы

В Broadcast карточка **Выдача** задаёт основной выход. В карточке
**Дополнительные потоки** можно добавить ещё два. Для каждого выхода отдельно
задаются адрес, протокол, признак «Стартовать со всеми» и, при необходимости,
собственный профиль транскодирования.

Общая кнопка **Начать эфир** поднимает одну программу и все отмеченные выходы.
После старта карточка **Потоки и ресурсы** в Encoding Monitor позволяет
остановить или снова запустить каждую дополнительную ветку без остановки
программы. Состав выходов фиксируется на старте сессии; добавить или удалить
выход во время эфира нельзя, потому что для него заранее создаётся безопасное
локальное зеркало.

Режимы и стоимость:

- `program` — единственный выход вместе с общей программой;
- `relay` — копия готового потока, без повторного кодирования;
- `transcode` — отдельный software FFmpeg со своим кодеком, разрешением и
  битрейтом.

Монитор показывает CPU, RAM и количество процессов общей программы и каждого
выхода. CPU считается как в `top`: 100% — одно логическое ядро. Для длительного
эфира держите сумму ниже примерно 70% доступных ядер и проверяйте `speed >= 1`
на худшем материале. Например, на 8 логических ядрах рабочий ориентир — менее
560% суммарного CPU эфирных цепочек. Это эксплуатационный запас, а не жёсткий
лимит программы.

Аппаратный кодировщик закреплён за общей программой. Дополнительные профили
транскодирования пока программные: это исключает борьбу нескольких FFmpeg за
одно устройство. Если мощности не хватает, используйте `relay`, уменьшите
профиль дополнительного выхода или вынесите независимую программу в отдельный
экземпляр/сервер.

Смена протокола сразу приводит форму к допустимому набору. RTMP/RTMPS фиксирует
H.264 + AAC и выключает/блокирует DVB, SCTE-35, multiaudio и MPEG-TS controls,
если в сессии нет UDP/SRT. SRT ничего из этого не блокирует: он несёт тот же
MPEG-TS, что UDP.
...
Секреты основного и дополнительных SRT/RTMP-выходов сохраняются в PostgreSQL
только в отдельном зашифрованном поле и вырезаются из открытого snapshot и
экспортируемого профиля.
...
Монитор намеренно лёгкий: 640 точек по ширине, 12,5 кадра в секунду, около
500 кбит/с, сегменты по 2 с, разбор без деблокинга. Он делит машину с эфиром, у
процессов эфира приоритет выше, и под нагрузкой монитор замирает первым — это
ещё не признак проблем в линии. Наоборот, погасший монитор при живом статусе
FFmpeg — повод проверить выход: зеркало берётся после TSDuck, и вставший
транспорт гасит обоих.
