...
- Группа тащится мышью целиком: её коробка ловит указатель так же, как узел.
  Пока группа не выбрана, она лежит **под** содержимым — щелчок по узлу берёт
  узел; у выбранной группы зона захвата поверх содержимого, и `Alt`-щелчок
  проваливается сквозь неё к узлу под курсором.
- Под холстом стоит строка выбранного: вид слоя, имя, размер и координаты **от
  центра кадра** в пикселях выбранной раскладки.
...
- Timeline показывает все layers, tracks раскрывает только там, где нужны;
  прокручивается сам список слоёв, панель времени вбок не ездит.
