...
При нескольких программах у каждой своя служба и база. Команды, имена служб и
порядок безопасного отключения приведены в
[multiple-programs.md](multiple-programs.md).
...
1. PostgreSQL dump;
2. `.env`, все `.env.program-*` и `instances.json`;
...
## Остановить станцию / поставить заново

Две команды из каталога установки. Обе идут по `instances.json`, поэтому
находят все программы сами — искать службы и порты руками не нужно.

```bash
npm run stop     # остановить все программы и кластер из комплекта
npm run reset    # снести станцию до состояния «до установки»
```

`npm run stop` останавливает службы всех программ и кластер PostgreSQL из
комплекта. Базы, `.env.shared`, `.env.program-*`, `instances.json` и сами службы
остаются: это пауза, а не удаление. Уже молчащая программа пропускается — она не
считается ошибкой.

`npm run reset` дополнительно удаляет каждую программу целиком (служба, база,
её `.env.program-N`), а затем убирает `.env.shared` и `instances.json`. После
него ставят заново: `npm run setup`. Программа в эфире удалению сопротивляется —
сначала Stop playout.

**Системный PostgreSQL не трогается ни одной из команд.** На машине он может
обслуживать не только FluxIO, и остановить его — значит уронить чужие базы.
`reset` удаляет ровно те базы, что названы в `.env` программ: искать их по маске
имени нельзя, рядом стоят чужие.

### Windows: «файл занят другим процессом»

Команда остановки возвращает управление раньше, чем процесс успел выйти —
`Stop-ScheduledTask` его не дожидается. Поэтому `npm run stop` ждёт, пока
станция действительно отпустит машину: сначала пока замолчат API-порты, затем
проверяет занятые `.node`. Если что-то осталось захваченным, команда называет
файлы и что с ними делать — это те же файлы, на которых потом обрывался бы
`npm ci` с `EPERM: unlink`.

Гасить чужие процессы команда не имеет права: порт может держать работающая
программа, и снять её — оборвать эфир ради установки. Решение остаётся за
оператором.
...
## Windows: автовход обязателен

На Windows кластер PostgreSQL и media-service поднимаются задачами планировщика
с триггером **«при входе пользователя»**, а не «при старте системы». Причина не
в удобстве: `-AtStartup` вправе зарегистрировать только администратор, а
PostgreSQL на Windows вдобавок отказывается работать из процесса с
административными правами — задача, зарегистрированная от администратора, не
подняла бы кластер и отказ переехал бы с установки на первую перезагрузку.

Следствие для эфирной машины: **без автовхода перезагрузка оставляет канал без
эфира** до тех пор, пока в машину физически не войдут. `autoResumeOnLaunch`
здесь не спасает — он поднимает эфир по чекпоинту, но сам живёт в media-service,
который до входа не запущен.

Включение: `Win+R` → `netplwiz` → снять «Требовать ввод имени пользователя и
пароля» → ввести пароль учётной записи оператора. Мастер напоминает об этом
последней строкой установки.

Проверять после каждой переустановки: перезагрузить машину, не входя в неё
руками, и убедиться, что эфир поднялся сам. Заодно это единственный способ
увидеть, успевает ли база стартовать раньше media-service — обе задачи
запускаются одним триггером, и порядок между ними не гарантирован.

На Linux вопроса нет: systemd поднимает и кластер, и службу до входа в систему.
