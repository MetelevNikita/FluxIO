...
Следом за `start on` идут объявления заголовка: языки переводов и определения
эфирных эффектов.

```text
defineAudioLanguage {rus} label {рус} clips {212}
defineAudioLanguage {eng} label {eng} clips {40}
```

Языки объявляются один раз на файл. Расписание открывают на другой машине и
через неделю, а пересканировать папку переводов при открытии нечем — это
ffprobe по каждому файлу расписания. Расписание без такого заголовка (сохранённое
до v8.0.2) не теряет языки: они собираются из самих строк `insertAudioTrack`.
...
Фильм, разрезанный под рекламу (кнопка «ножницы» в панели плейлиста), пишется
частями — отрезками одного файла. Точка входа стоит в угловых скобках перед
длительностью, тем же синтаксисом, что у AirSheet; у первой части она нулевая и
не пишется. Название части, заданное оператором, едет строкой `insertName`, а
пометка между частями — отдельной строкой `comment`:

```text
insertName {Фильм · часть 1}
movie 00:45:00.00 /media/Film.mp4
comment {Рекламный блок}
insertName {Фильм · часть 2}
movie <00:45:00.00> 00:45:00.00 /media/Film.mp4
```

Пометка хронометража не занимает и в эфир не уходит. Части одного фильма
узнаются по стыку — строка того же файла, начинающаяся там, где кончилась
предыдущая, — и в плейлисте красятся розовым. Метки SCTE-35 у части считаются от
начала файла, а не от начала части.
...
Выбранная папка переводов и найденные в ней языки переживают перезапуск: путь
лежит в настройках, список языков — в workspace, а сами дорожки после
восстановления сессии ищутся на диске заново (пути к файлам в снимке не
хранятся). Сканирование при запуске идёт только если папку когда-то открывали.
...
graphics, broadcast effects, SRT, дорожки перевода и список их языков.
Абсолютные пути записываются буквально.
