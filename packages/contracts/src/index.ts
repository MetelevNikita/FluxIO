...
export const dynamicTitleSettingsSchema = z.object({
  ...
  /**
   * Значения полей сцены, заданные оператором: ключ поля → строка.
   *
   * Поля объявляет сама сцена, и их может быть сколько угодно — пара
   * «строка плюс подпись» перестала описывать титр, как только плашку стало
   * можно собрать самому. В режиме файла задания эти значения остаются
   * резервными: ролик, которому в JSON записи не нашлось, покажет их.
   */
  fieldValues: z.record(z.string().min(1).max(128), z.string().max(2_000)).default({}),
