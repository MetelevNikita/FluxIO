...
export const sceneTransformSchema = z.object({
  ...
  /**
   * Как раскрытие выглядит.
   *
   * `wipe` — шторка: окно растёт от точки среза, картинка стоит на месте.
   * `slide` — выезд: маска стоит рамкой узла, а картинка выезжает из-под её
   * края — оттуда же, где стоит точка среза. Разница видна на строке: у
   * шторки буквы проявляются по одной на месте, у выезда строка выползает
   * целиком, как из-за края плашки.
   *
   * По умолчанию `slide`: «раскрытие» на слух и на глаз — это выезд, и
   * шторка под этим словом оператора обманывала. Шторка никуда не делась —
   * она осталась готовым входом «Развёртка» и этим же переключателем.
   */
  revealMode: z.enum(["wipe", "slide"]).default("slide"),
...
export const sceneTextStyleSchema = z.object({
  ...
  /**
   * Вписывать ли надпись в собственную рамку узла.
   *
   * Значение поля приходит извне — из файла задания, из `.json` выгрузки, —
   * и длина его не выбирается: «Иван» и «Александр Константинопольский»
   * приезжают в одну и ту же плашку. Без подгонки длинная строка либо
   * уезжает за край кадра, либо растягивает привязанную подложку через
   * половину экрана.
   *
   * Уменьшается **кегль**, а не ширина: сжатие по горизонтали ломает буквы,
   * и это видно на выходе, а не в редакторе.
   */
  autoFit: z.boolean().default(false),
  /**
   * Ниже этой доли кегля не сжимать.
   *
   * Строка, ужатая в десять раз, читается не лучше срезанной, а мелкий кегль
   * на эфирном кодировщике рассыпается в кашу. Упёршись в минимум, надпись
   * обрезается по рамке — это по-прежнему честнее, чем выпустить её поверх
   * соседних элементов.
   */
  autoFitMinScale: z.number().min(0.1).max(1).default(0.4),
