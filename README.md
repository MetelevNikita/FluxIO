...
В v8.0.0 перестроена библиотека эффектов. Уровень 3 перестал быть отдельным
элементом списка: графика стала свойством конкретного эффекта, поэтому общий
импорт и выбор пресета из выпадающего списка убраны. Оформление стало явным
выбором — шаблон или плашка, которую эффект рисует сам. Шаблоны титров
принимаются только из FluxIO Title Studio, Animation In/Out требует alpha-медиа,
Stinger — `.mov` с альфой или последовательность `.png` с частотой кадров,
заданной оператором. Незавершённый эффект виден в списке, и применить его
нельзя. Пустое расписание больше не отказ на старте: в линию уходят цветные
полосы, они же стоят в мониторе до запуска. Мастер установки первым вопросом
спрашивает про офлайн-режим и хранит только две последние резервные копии
`.env`. Карта потока работы с эффектами — `docs/effects-flow-map.html`.
