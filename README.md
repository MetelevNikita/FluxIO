...
В v8.0.0 у FluxIO появились **собственные титры**. Связка с After Effects и
Lottie убрана целиком: шаблон — это сцена, дерево узлов внутри самого эффекта,
и рисует её одна общая функция, которую вызывают и редактор, и эфир. Причина
не в удобстве, а в конкретном отказе на эфире: раньше плашка была отрендеренным
файлом, а текст поверх неё рисовал FFmpeg отдельным фильтром — две сущности с
разными часами, и текст успевал появиться раньше своей подложки. Теперь плашка
и текст рисуются одним проходом, одним кадром.

Заодно: один шаблон работает на все четыре раскладки (4:3, 16:9, HD, UHD) и обе
частоты (25 и 50 полей) — пересчёт идёт от долей кадра; промежуточные файлы
исчезли, значения подставляются на лету; предпросмотр совпадает с эфиром, потому
что кода отрисовки ровно один. Рисуется не весь кадр, а только область показа:
полный кадр 2160 RGBA — это 829 МБ/с, и в реальное время он не укладывается.

Оформление перестало быть отдельным элементом списка: у большинства эффектов оно
живёт сценой, а Animation In/Out и Stinger по-прежнему требуют готовое
alpha-медиа. Незавершённый эффект виден в списке, и применить его нельзя. Пустое
расписание больше не отказ на старте: в линию уходят цветные полосы, они же стоят
в мониторе до запуска. Мастер установки первым вопросом спрашивает про
офлайн-режим и хранит только две последние резервные копии `.env`. Карта потока
работы с эффектами — `docs/effects-flow-map.html`.

В v7.0.17 переработан полный цикл Effects: пакетный импорт сохраняет исправные
файлы и объясняет пропуски, JSON задания восстанавливается вместе с сессией,
сопоставление понимает имена файлов, а Next Program и Stinger проходят границу
Current → Future. Узкие Lottie-композиции теперь используют одинаковую
`contain`-геометрию для плашки и живого текста; стингер проверяется через
FFprobe, а инспектор получил безопасный черновик, поиск шрифтов и доступную
панель применения.

В v7.0.16 Animation In/Out получил пакетное применение из JSON: поле `title`
сопоставляется с именем ролика, произвольные ключи — с одноимёнными Text Layer,
а одна операция раскладывает титры по Current и Future. Повторы ролика
поддерживаются, дубли `title` блокируются, повторное применение не копит слои.

В v7.0.15 добавлен Flux Title Exporter — панель After Effects, которая выбирает
композицию и динамические Text Layer, связывает отзывчивые Shape Layer и
экспортирует готовый к импорту Lottie JSON. FluxIO читает метаданные экспортёра,
показывает только выбранные поля и сохраняет предупреждения совместимости.
Само расширение развивается как отдельный companion-проект `FluxIO-Title-Studio`,
а не как часть исходников эфирного приложения.

В v7.0.14 появился единый JSON Parser: он разбирает плоские и вложенные данные,
сопоставляет их с Text Layer любого пользовательского Lottie-шаблона и показывает,
какие поля имеют отзывчивую подложку `fit:<поле>`. Graphics UI собран в поток
«Шаблон → Данные → Эфир → Оформление».

В v7.0.13 `Next program` использует живую надпись FFmpeg и адаптивную
Lottie-подложку `fit:<поле>`, а для `Clock / countdown` добавлены явная
настройка резиновой подложки и готовые тестовые пресеты.

В v7.0.12 Channel logo принимает MOV, GIF и другие видеоформаты, а выбранный
Lottie JSON автоматически рендерится в alpha-MOV перед эфиром.

В v7.0.11 появился `Dynamic title`: Lottie рисует анимированный декор и
адаптивную подложку, а произвольный текст из интерфейса или файла задания
накладывается отдельно через FFmpeg.

В v7.0.10 полоса бегущей строки видна в предпросмотре, а применение
предупреждает, если привязанная строка уедет за пределы плашки.

В v7.0.9 бегущая строка едет внутри заданной полосы и не выходит за пределы
Lottie-плашки.

В v7.0.8 плашка и живое значение эфирного эффекта показываются в плейлисте
одним чипом, а карточки эффектов переставляются перетаскиванием.

В v7.0.7 часы, отсчёт и бегущая строка встают внутрь текстового поля
Lottie-плашки, наследуя её положение, кегль, цвет и выключку.

В v7.0.6 панель настроек эффекта всегда помещается в видимую область, у Ticker
и Clock появилась подпись в текстовом поле пресета, а чипы графики в плейлисте
уходят в колонки без роста высоты строки.

В v7.0.5 поля инспектора эффектов не тормозят при вводе, кнопка Save переносит
правки в уже назначенные ролики, а Lottie-пресет можно выбрать и подгрузить
в любом эфирном эффекте, включая стингер.

В v7.0.4 эффекты, отсчитываемые от конца ролика, встают правильно и на
роликах из импортированного расписания.

В v7.0.3 имена системных шрифтов читаются правильно, ключ текстового поля
выбирается из реального набора Lottie-пресета, обратный отсчёт умеет считать
до конца ролика, а media-service больше не замирает на рендере графики.

В v7.0.2 вкладка Effects остаётся отзывчивой во время эфира, бегущая строка
двигается и снимается прямо из плейлиста, берёт новости из RSS и рисуется
выбранным системным шрифтом с проверкой кириллицы. У каждого эфирного эффекта
...
- отдельная вкладка `Effects`: библиотека проекта PNG/WebP/MOV/MP4 для оформления файлом и собственные титры-сцены для всего остального; предпросмотр рисуется тем же кодом, что и эфир, поэтому совпадает с выходом; назначение всему проекту либо выбранному ролику;
...
- предпросмотр эффекта в 4:3, 16:9, HD и UHD: одна сцена пересчитывается под
  раскладку сама, отдельных файлов под каждое разрешение не нужно;
